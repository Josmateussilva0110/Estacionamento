...
export function TimeRangePicker({ value, onChange, label, error }: TimeRangePickerProps) {
  ...
  useEffect(() => {
    if (value) setTemp({ start: value.start ?? "", end: value.end ?? "" })
  }, [value])
          ...
          return (
            <>
              ...
              <Popover.Button
                ...
                `}
              >
                <span className={`text-sm flex-1 text-left ${hasValue ? "text-white font-medium" : "text-slate-400"}`}>
                  {hasValue ? `${value.start} — ${value.end}` : "Selecione o horário"}
                </span>
                <Clock className={`w-4 h-4 shrink-0 transition-colors ${hasValue ? "text-blue-400" : "text-slate-500"}`} />
              </Popover.Button>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <input
                          type="time"
                          value={temp.start}
                          onChange={(e) => setTemp((prev) => ({ ...prev, start: e.target.value }))}
                          className="
                      ...
                      <div className="space-y-1.5">
                        ...
                        <input
                          type="time"
                          value={temp.end}
                          onChange={(e) => setTemp((prev) => ({ ...prev, end: e.target.value }))}
                          className="
                            w-full border border-slate-600/50 rounded-xl px-4 py-2.5 text-sm
                            text-white bg-slate-700/50
                            focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50
